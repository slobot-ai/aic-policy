...
Same two-pass `RectangleClassifier` fit, with **SC-port** half-extents on both axes from :data:`vision.rectangle3d_detector.SC_PORT_FOA_MESH_AABB` (see **§5** intro), not the **0.05 m** logo box used on observation **2**. **Blue SC port only:** the optimizer uses **center camera** masks only (`classifier_camera_indices=(1,)` on `RectangleVisionInput`); **initializer** also uses the **center** view only for centroiding (see **§5.4**). Left and right still run **segmentation** for masks and PNG dumps. `mean_hard_iou` on third-observation candidates is the center-view classifier metric (multiview top-view PNGs still draw all three footprints).

`RectangleVisionInput.classifier_native_crop_around_initializer` is **on** for this pass: the initializer still uses quarter-res masks, but the classifier sees **native-resolution** mask crops (and shifted intrinsics) at the same linear scale as `image_scale`, centered on the initializer merged centroid—so the thin blue port is not blurred by a global downscale before IoU optimization.
